...
你需要起一个`Web服务器`来访问，双击本地运行 `index.html` 并不会执行 `type=module` 标签我们可以对 `script` 标签的 `type` 属性加上 `module`先定义两个模块
...
可以看到 `es module` 在`浏览器`与`node`中兼容性差与实验功能的我们大多时候在 打包工具 中使用
