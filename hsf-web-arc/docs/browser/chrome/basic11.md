...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMXOVSzY8UuNj69QsXfG9LnXiaflONt3jXDljoiaZpdicsL1stEy0RdjJrQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMh2huR844oSr6mPMufme2uUK0UIGkhXD8GJiclJm7e3ty0aQxQszo3nQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
举个例子，google 的官网是 [www.google.com](http://www.google.com) ，而 google 的 ip 地址是 216.58.200.228 ，这两个地址你在 URL 上输入哪个都能访问，但是 IP 地址不好记忆，而 [google.com](http://google.com) 简单明了。DNS 就相当于是我们几年前使用的家庭电话薄，比如你想给 cxuan 打电话，你有可能记不住 cxuan 的电话号码，此时你需要查询电话薄来找到 cxuan 的电话号码。
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMwlYyosMhOibXP5pev0LAJTSffoPvnE1iaOY1kPmLxMLcic9a0Qsz1M3aA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1

如果上面四个步骤中都不存在 DNS 记录，那么就表示不存在 DNS 缓存，这个时候就需要发起 DNS 查询，以查找目标网址（本示例中是 [maps.google.com](http://maps.google.com)）的 IP 地址。

## **发起 DNS 查询**

如上所述，如果想要使我的计算机和 [maps.google.com](http://maps.google.com) 建立连接并进行通信的话，我需要知道 [maps.google.com](http://maps.google.com) 的 IP 地址，由于 DNS 的设计原因，本地 DNS 可能无法给我提供正确的 IP 地址，那么它就需要在互联网上搜索多个 DNS 服务器，来找到网站的正确 IP 地址。
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMBG7eVB9SDJVttcHLDTqfWtfibhuSTgSMLJB3yyYHzn8qzmtrmWeEVMQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1

https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMAbocPiaUYHibJBpe6OZgyEvER2egrITp6eJsT6L9941ib7DmVaBXQiad0w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMO1Pavu22IGw0D1iaHAXUla9etamian2rCLJT2KDIQwnTSlB86LMRYfpA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOM7MZlRia5zOV9LSmLS6m7CmnKkE34bqic07icN9yUQXBkv9CNxIiaELib4VQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
所以对于 [maps.google.com](http://maps.google.com) 这个域名来说，如果 ISP 维护的服务器没有 DNS 缓存记录，它就会向 DNS 根服务器地址发起查询，根名称服务器会将其重定向到 .com 顶级域名服务器。.com [顶级域名服务器会将其重定向到google.com](http://xn--google-o88i64n3xbh6an0iqb286c6ofpzqltbc10hi12flbtd921a.com) 权威服务器。[google.com](http://google.com) 名称服务器将在其 DNS 记录中找到 [maps.google.com](http://maps.google.com) 匹配的 IP 地址，并将其返回给您的 DNS 解析器，然后将其发送回你的浏览器。
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSeow2O1jOX0Up1tBMpiatGBNbt4bYD1MeCmQ8XzowazJWV1uDlSr60meJP0FX1FM7CtBuyU0Q8vPg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1

主机 A 想要获取主机 B 的 MAC 地址，通过主机 A 会通过`广播` 的方式向以太网上的所有主机发送一个 `ARP 请求包`，这个 ARP 请求包中包含了主机 A 想要知道的主机 B 的 IP 地址的 MAC 地址。

https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSeow2O1jOX0Up1tBMpiatGBFThlhlqiaUvvy837Qpt9P6Qc2Mxm1u36iargOicSsfO0P6f7mZkcRKrTA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1

主机 A 发送的 ARP 请求包会被同一链路上的所有主机/路由器接收并进行解析。每个主机/路由器都会检查 ARP 请求包中的信息，如果 ARP 请求包中的`目标 IP 地址` 和自己的相同，就会将自己主机的 MAC 地址写入响应包返回主机 A

https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMR4IQl7K8IpF46attMxI8wTVo6D1cM1r0wibQpHcHYAicPVeyAMzHjmMg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSeow2O1jOX0Up1tBMpiatGBJDXMSuvxLichqrqBBjjJjNBIx88QHTkvNOenovuY3ao2EnIws3sGicoQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMlbmevRPAXLStiaL8hqhC7DWqtNysj77fZkMD8pr68tpD1dxwibk1DRqQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMGl01V5aUbFjhQSGIhHxxnPVDiaDAJCDKeR2R34TBAsicH3ODm7q1da7A/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMH08dRnxmfM1lYEw23Iy4BnKZfOggA3Gu0eWajj7CSE5SLuEblLiagcg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMpxsPz3akJFyzMpUqpFnVWaf7zqAyUAZVkmJJIFx5LDvQv0Cv5433sA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSyyk2RND0WvF0P6K81kFSva8hEhYPQibbBibMWMJlvV6UYaDfZBGW5RX4oiaYAu3vqN77CYbPJbqgXA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
一旦 TCP 连接建立完成后，就开始直接传输数据办正事了！此时浏览器会发送 `GET` 请求，要求目标服务器提供 [maps.google.com](http://maps.google.com) 的网页，如果你填写的是表单，则发起的是 `POST` 请求，在 HTTP 中，GET 请求和 POST 请求是最常见的两种请求，基本上占据了所有 HTTP 请求的九成以上。

除了请求类型外，HTTP 请求还包含很多很多信息，最常见的有 Host、Connection 、User-agent、Accept-language 等

https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMLoSv5DBKibZtwZYqgaPGBOZSsKsiaw9no8F3MXuaU4969r20XoCibkibSg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1
...
https://mmbiz.qpic.cn/mmbiz_png/A3ibcic1Xe0iaSXEH4kiaoibyxKicUjVYbRmOMhQcjCEgSqqQAuNH6cwaqvaMSfFhvJgg8ZQLWqEQCia4gLoIRmibUOiaibQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1

## **浏览器显示 HTML 的相关内容**

浏览器会分阶段显示 HTML 内容。首先，它将渲染裸露的 HTML 骨架。然后它将检查 HTML 标记并发送 GET 请求以获取网页上的其他元素，例如图像，CSS 样式表，JavaScript 文件等。这些静态文件由浏览器缓存，因此你再次访问该页面时，不用重新再请求一次。最后，您会看到 [maps.google.com](http://maps.google.com) 显示的内容出现在你的浏览器中。
